...
2. Pick a location for your SQD Network key file. The location must be outside of the data directory.
   ...
   ⚠️ **Warning:** Ensure that that the key will not be deleted accidentally and cannot be accessed by unauthorized parties. [Or else](#what-are-the-consequences-of-losing-my-key-file--getting-it-stolen).
...
4. Run the setup script. Supply the data directory path and the key path to it:
   ...
   The script will prompt you for an UDP port to use for P2P communication and give you an option to set your public IP address in worker config statically. _Most users should not set the IP address here_ since setups with automatic IP discovery are more robust.
   ...
   _Please copy your peer ID_, as it will be needed for [on-chain worker registration](#worker-registration).
...
Proceed to [Worker registration](#worker-registration).
...
2. Connect your Primary Wallet.
   ...
   ![Connect wallet button](/images/network/participate/worker_registration_wallet.png)

3. Go to the Workers tab and press the "Add Worker" button. You should see a worker registration form:

   ![Worker registration form](/images/network/participate/worker_registration_form.png)
...
4. Fill the form and submit it by signing a transaction:
   ...
   - Use the peer ID you copied at step 4 of [Configuring your setup](#configuring-your-setup).
...
Make sure you're still in the folder where you executed `setup_worker.sh` during [configuration](#configuring-your-setup) before proceeding.
...
If any of that happens, unregister your worker (on the ["Workers" tab of network.subsquid.io](https://network.subsquid.io/workers)), [generate](#configuring-your-setup) a new key file and [register](#worker-registration) your new peer ID.
