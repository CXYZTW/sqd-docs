...
2. Contract events retrieval:
   ...
   ```
   contract = Contract('0xdac17f958d2ee523a2206206994597c13d831ec7', abi='usdt.json')df = contract.Transfer.query(    '*',    start_block=18_000_000,    stop_block=18_100_000,    engine_to_use='subsquid')
   ```
...
<Warning>
  ...
  ```
  from ape_subsquid import get_network_heightget_network_height() # currently 19212115 while the chain is at 19213330
  ```
  ...
  ```
  subsquid_height = get_network_height()df = chain.blocks.query(    '*',    start_block=19_000_000,    stop_block=subsquid_height,    engine_to_use='subsquid')
  ```

  To get the latest data, retrieve the tail with the default engine and append it to the dataframe:

  ```
  taildf = chain.blocks.query('*', start_block=subsquid_height+1)df = pd.concat([df, taildf], ignore_index=True, copy=False)
  ```
</Warning>
...
## Networks support[​](#networks-support "Direct link to Networks support")
...
## Extra functionality[​](#extra-functionality "Direct link to Extra functionality")

### Low-level queries[​](#low-level-queries "Direct link to Low-level queries")
...
### Account history retrieval[​](#account-history-retrieval "Direct link to Account history retrieval")
...
```
accountHistory = chain.history._get_account_history('vitalik.eth')accountHistory.query('value', engine_to_use='subsquid')
```

[Edit this page](https://github.com/subsquid/docs/edit/master/docs/apeworx.md)
